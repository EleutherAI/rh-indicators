...
def run_analysis(
    ...
    # Use the minimum number of checkpoints across runs
    exploit_ckpts = sorted(exploit_df["checkpoint"].unique())
    control_ckpts = sorted(control_df["checkpoint"].unique())
    ...
    for cutoff_idx in range(max_cutoff_idx + 1):
        ...
        print(
            ...
            f"exploit ckpt {exploit_cutoff_ckpt}, control ckpt {control_cutoff_ckpt} ---"
        )

        for metric in METRICS:
            # Build feature dataframe across both runs
            rows = []
            for run_df, run_label in [
                ...
                run_ckpts = sorted(run_df["checkpoint"].unique())
                log_final_ckpt = np.log(run_ckpts[-1])
            ...
            result_base = {
                ...
                "exploit_cutoff_ckpt": exploit_cutoff_ckpt,
                "control_cutoff_ckpt": control_cutoff_ckpt,
                "metric": metric,
            }
            ...
            # --- Approach 2: Tuned projection (1 param) ---
            tuned_result = tuned_projection_auc(features_df)

            all_results.append(
                {
                    **result_base,
                    "model": "tuned_projection",
            ...
            # --- Approach 3: Level + slope logistic (3 params) ---
            logistic_result = logistic_loo(features_df)

            all_results.append(
                {
                    **result_base,
                    "model": "logistic_level_slope",
            ...
            # Save LOO predictions for the logistic model
            if "loo_probs" in logistic_result:
                ...
                pred_df.to_csv(
                    output_dir
                    / f"logistic_loo_cutoff{cutoff_idx}_{metric}_predictions.csv",
                    index=False,
                )
    ...
    plot_feature_separation(exploit_df, control_df, output_dir)
...
def plot_feature_separation(
    exploit_df: pd.DataFrame, control_df: pd.DataFrame, output_dir: Path
) -> None:
    ...
    for ax, cidx in zip(axes, cutoffs_to_plot):
        ...
        control_vals = []
        for et in sorted(INTENTIONAL_TYPES):
            for run_df, vals_list in [
                (exploit_df, exploit_vals),
                (control_df, control_vals),
            ]:
                feats = compute_features_at_cutoff(run_df, et, cidx, metric)
                if feats is not None:
                    vals_list.append(feats["level"])

        exploit_vals = np.array(exploit_vals)
        control_vals = np.array(control_vals)

        ax.scatter(
            np.zeros(len(control_vals)) + np.random.normal(0, 0.05, len(control_vals)),
            control_vals,
            c="C0",
            alpha=0.7,
            s=40,
            label="Control (no exploit)",
        )
        ax.scatter(
            np.ones(len(exploit_vals)) + np.random.normal(0, 0.05, len(exploit_vals)),
            exploit_vals,
            c="C3",
            alpha=0.7,
            s=40,
        ...
        ax.set_xticks([0, 1])
        ax.set_xticklabels(["Control", "Exploit"])
        ax.set_title(
            f"Cutoff idx {cidx}\n"
            f"(exploit ckpt {exploit_ckpts[cidx]}, control ckpt {control_ckpts[cidx]})"
        )
...
def main():
    ...
    def _run_all(output_dir: Path) -> None:
        ...
        results = run_analysis(exploit_df, control_df, output_dir)
