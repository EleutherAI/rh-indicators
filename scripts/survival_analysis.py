...
def compute_time_to_exploit(
    ...
    - censored: True if problem was never exploited at baseline
    - censor_checkpoint: checkpoint value used for censored observations (default: 100)
    """
    ...
    for (task_id, exploit_type), group in baseline.groupby(["task_id", "exploit_type"]):
        ...
        if first_exploit_checkpoint is not None:
            results.append({
                "task_id": task_id,
                "exploit_type": exploit_type,
                "time_to_exploit": first_exploit_checkpoint,
                "censored": False,
            })
        else:
            # Right-censored: never exploited at baseline
            # Use fixed censor checkpoint (100) per user request
            results.append({
                "task_id": task_id,
                "exploit_type": exploit_type,
...
def prepare_survival_data(
    ...
    survival = compute_time_to_exploit(df, checkpoints)
...
def main():
    ...
    print(f"\nPreparing survival data (early checkpoint: {args.early_checkpoint}, method: {args.sensitivity_method})...")
    survival_data = prepare_survival_data(combined, args.early_checkpoint, checkpoints, args.sensitivity_method)
    print(f"Survival data: {len(survival_data)} problems")
    print(f"  Events (exploited): {survival_data['event'].sum()}")
    print(f"  Censored (never exploited): {(~survival_data['event'].astype(bool)).sum()}")
